...
class CompleteGlowupAIService(GlowupAIService):
    ...
    def __init__(
        self, db: FullDatabase, settings: Any = None, photos: Any = None
    ) -> None:
        super().__init__(
    ...
    def record_data_collection_consent(
        ...
        return self.user_svc.record_data_collection_consent(
            user_id, granted, policy_version
        )
    ...
    def _audit(
        ...
        return self.user_svc._audit(
            action, subject_type, subject_id, actor_id, metadata
        )
    ...
    def history(self, user_id: str, vertical: str = "skin") -> list[dict[str, Any]]:
        return self.capture_svc.history(
    ...
    def add_label(
        ...
        return self.capture_svc.add_label(
            user_id, photo_id, label_type, value, confidence, notes
        )

    def reprocess(self, user_id: str, model_version: str) -> dict[str, Any]:
        return self.capture_svc.reprocess(
            user_id, model_version, self.jobs, self._audit
        )
    ...
    def export_training_dataset(
        ...
        return self.capture_svc.export_training_dataset(
            output_dir, min_quality, max_samples
        )
    ...
    def record_engagement(
        ...
        return self.analytics_svc.record_engagement(
    ...
    def add_context_event(
        ...
        return self.analytics_svc.add_context_event(
            user_id, event_type, value, occurred_at, notes
        )
    ...
    def experiment(self, experiment_id: str, user_id: str) -> dict[str, Any]:
        return self.guidance_svc.experiment(
            experiment_id, user_id, history_fn=self.history
        )

    def experiments(self, user_id: str) -> list[dict[str, Any]]:
        return self.guidance_svc.experiments(
            user_id, require_premium_fn=self.require_premium
        )
    ...
    def ask(
        self, user_id: str, question: str, thread_id: str | None = None
    ) -> dict[str, Any]:
    ...
    def qna_history(self, user_id: str) -> list[dict[str, Any]]:
        return self.guidance_svc.qna_history(
            user_id, require_premium_fn=self.require_premium
        )

    def scan_shelf(self, user_id: str, image_bytes: bytes) -> dict[str, Any]:
        return self.guidance_svc.scan_shelf(
            user_id, image_bytes, record_engagement_fn=self.record_engagement
        )
    ...
    def discover(self, user_id: str) -> dict[str, Any]:
        return self.commerce_svc.discover(
            user_id, require_premium_fn=self.require_premium
        )
    ...
    def add_offer(
        ...
        return self.commerce_svc.add_offer(
            product_id, merchant, url, price_cents, currency
        )
    ...
    def click_offer(self, user_id: str, offer_id: str) -> dict[str, Any]:
        return self.commerce_svc.click_offer(
    ...
    def product_detail(self, user_id: str, product_id: str) -> dict[str, Any]:
        return self.commerce_svc.product_detail(
            user_id, product_id, self.is_premium(user_id), offers_fn=self.offers
        )
