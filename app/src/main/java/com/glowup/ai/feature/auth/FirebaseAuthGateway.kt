...
object FirebaseAuthGateway {
    fun instanceOrNull(): FirebaseAuth? = try {
    ...
    fun signOut() { instanceOrNull()?.signOut() }
    ...
    suspend fun signInWithGoogle(activity: Activity): Result<FirebaseUser> {
        ...
        return try {
            val provider = OAuthProvider.newBuilder("google.com")
                ...
                .build()
            val task: Task<AuthResult> = auth.pendingAuthResult
    ...
    private fun validateCredentials(email: String, password: String): Throwable? = when {
        email.isBlank() || !android.util.Patterns.EMAIL_ADDRESS.matcher(email).matches() ->
            InvalidAuthInputException("Enter a valid email address.")
        password.length < 6 -> InvalidAuthInputException("Choose a password with at least 6 characters.")
        else -> null
    }
    ...
    fun friendlyMessage(t: Throwable): String = when (t) {
        is FirebaseUnavailableException -> "Sign-in isn't available on this build yet. Please try again later."
        ...
        is FirebaseAuthInvalidCredentialsException -> "That email or password looks incorrect."
        is FirebaseAuthUserCollisionException -> "An account already exists for that email. Try signing in instead."
        is FirebaseAuthInvalidUserException -> "We couldn't find an account for that email."
...
class InvalidAuthInputException(val userMessage: String) : IllegalArgumentException(userMessage)