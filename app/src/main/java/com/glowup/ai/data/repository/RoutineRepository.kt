...
class RoutineRepository
    ...
    constructor(
        ...
        suspend fun getProduct(
            productId: String,
            userId: String,
        ): GlowResult<ProductDetail> = apiCall { api.getProduct(productId, userId).toDomain() }

        suspend fun getIngredientExplainer(
            productId: String,
            userId: String,
        ): GlowResult<IngredientExplainer> = apiCall { api.getIngredientExplainer(productId, userId).toDomain() }

        suspend fun predictProduct(
            productId: String,
            userId: String,
        ): GlowResult<ProductPrediction> = apiCall { api.predictProduct(productId, userId).toDomain() }

        suspend fun purchaseGuidance(
            userId: String,
            request: PurchaseGuidanceRequest,
        ): GlowResult<PurchaseGuidance> = apiCall { api.purchaseGuidance(userId, request.toDto()).toDomain() }
        ...
        ): GlowResult<ConfoundCheck> = apiCall { api.confoundCheck(userId, excludeProductId).toDomain() }

        suspend fun logRoutineEvent(request: RoutineEventRequest): GlowResult<RoutineEvent> =
            mutations
                .run("routine_event:${request.userId}:${request.productId}:${request.action}") {
        ...
        suspend fun submitShelfScan(
            userId: String,
            imageBase64: String,
        ): GlowResult<String> = apiCall { api.submitShelfScan(userId, ShelfScanCreateRequestDto(imageBase64)).jobId }
        ...
        suspend fun getShelfScanStatus(
            userId: String,
            jobId: String,
        ): GlowResult<ShelfScanJob> = apiCall { api.getShelfScanStatus(userId, jobId).toDomain(jobId) }

        suspend fun confirmShelfScan(
            userId: String,
            jobId: String,
            selections: List<ShelfScanSelection>,
        ): GlowResult<List<Product>> =
            mutations
                .run("shelf_scan_confirm:$jobId") {
                    apiCall {
                        api
                            .confirmShelfScan(
                                userId,
                                ...
                                com.glowup.ai.data.remote.dto
                                    .ShelfScanConfirmRequestDto(selections.map { it.toDto() }),
                            ).map { it.toDomain() }
                    }
                }.onSuccess {
