...
class ApiErrorMapperTest {
    ...
    @Test
    fun `403 with consent message maps to ConsentRequired`() = runTest {
        server.enqueue(
    ...
    @Test
    fun `403 with requires Premium message maps to PremiumRequired with feature`() = runTest {
        server.enqueue(
            MockResponse().setResponseCode(403).setBody(
    ...
    @Test
    fun `400 with structured quality object maps to CaptureQualityRejected with coaching`() = runTest {
        server.enqueue(
        ...
        val result = apiCall {
            api.createCapture(
                com.glowup.ai.data.remote.dto.CaptureCreateRequestDto(userId = "user-1", imageBase64 = "abc"),
            )
        }
    ...
    @Test
    fun `422 validation array maps loc to field name`() = runTest {
        server.enqueue(
        ...
        val result = apiCall {
            api.askQna("user-1", com.glowup.ai.data.remote.dto.QnaCreateRequestDto(question = ""))
        }
    ...
    @Test
    fun `400 not found string maps to NotFound`() = runTest {
    ...
    @Test
    fun `400 already exists string maps to Conflict`() = runTest {
        server.enqueue(MockResponse().setResponseCode(400).setBody("""{"detail":"barcode already exists"}"""))

        val result = apiCall {
            api.createProduct(com.glowup.ai.data.remote.dto.ProductCreateRequestDto(name = "Serum", barcode = "123"))
        }
    ...
    @Test
    fun `204 delete finishes without parsing a body`() = runTest {
