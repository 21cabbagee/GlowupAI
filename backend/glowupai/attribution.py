...
class AttributionEngine:
    ...
    def evaluate_user(self, user_id: str) -> list[AttributionResult]:
        products = self.db.fetchall(
            """SELECT DISTINCT p.id, p.name, p.stabilization_days FROM products p
               JOIN routine_events e ON e.product_id = p.id WHERE e.user_id = ?""",
            (user_id,),
        )
    ...
    def evaluate_product(self, user_id: str, product_id: str) -> AttributionResult:
        ...
        events = self.db.fetchall(
            "SELECT * FROM routine_events WHERE user_id = ? AND product_id = ? ORDER BY timestamp",
            (user_id, product_id),
        )
        captures = self.db.fetchall(
            """SELECT c.captured_at, m.blemish_count, m.redness_score, m.darkspot_area,
               ...
               WHERE c.user_id = ? AND c.status = 'accepted' ORDER BY c.captured_at""",
            (user_id,),
        )
        if not events:
            return self._result(
        ...
        start_event = next(
            (
                ...
                if event["action"] in ("start", "change")
            ),
            events[0],
        )
        ...
        end_event = next(
            (
                event
                for event in events
                if event["action"] == "stop" and parse_time(event["timestamp"]) > start
            ),
            None,
        )
        ...
        before = [
            row
            for captured, row in typed_captures
            if captured < start and (start - captured).days <= 45
        ]
        after = [
            row
            for captured, row in typed_captures
            if captured >= stable_at and (end is None or captured <= end)
        ]
        if end:
        ...
        other_events = self.db.fetchall(
            """SELECT e.timestamp, p.name FROM routine_events e JOIN products p ON p.id = e.product_id
               WHERE e.user_id = ? AND e.product_id <> ? AND e.action IN ('start','change')""",
            (user_id, product_id),
        )
        confounded_by = [
            row["name"]
        ...
        if confounded_by:
            return self._result(
                ...
                {
                    **base_evidence,
                    "reason": "another routine variable changed during the stabilization window",
                },
            )
        if not after:
            return self._result(
                product,
                "evidence_unclear",
                {**base_evidence, "reason": "not enough post-stabilization captures"},
            )
        if not before:
            return self._result(
                product,
                "evidence_unclear",
                {**base_evidence, "reason": "no comparable pre-product capture"},
            )
        ...
        improvements = {
            metric: value for metric, value in normalized.items() if value >= 1.5
        }
        worsening = {
            metric: value for metric, value in normalized.items() if value <= -1.5
        }
        ...
        evidence = {
            **base_evidence,
            ...
            "evidence_window_start": iso(
                min(parse_time(row["captured_at"]) for row in captures),
            ),
            "evidence_window_end": iso(
                max(parse_time(row["captured_at"]) for row in captures),
            ),
        }
        if worsening and not improvements:
            ...
            label = "keep"
            evidence["reason"] = (
                "all observed movement remains within the capture noise floor"
            )
        else:
            label = "evidence_unclear"
            evidence["reason"] = (
                "capture confidence or sample size is too low for a reliable comparison"
            )
