...
class PrivacyRepository @Inject constructor(
    ...
    suspend fun exportData(userId: String): GlowResult<ExportBundle> =
        apiCall { api.exportUser(userId).toDomain() }
     ...
     */
    suspend fun getAnalytics(userId: String): GlowResult<Analytics> =
        apiCall { api.getAnalytics(userId).toDomain() }
