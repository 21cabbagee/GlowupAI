...
class HomeRepository
    ...
    constructor(
        ...
        private fun dashboardRoomKey(
            userId: String,
            plan: com.glowup.ai.domain.model.Plan,
            vertical: String,
        ) = "$userId:${plan.name.lowercase()}:$vertical"

        private suspend fun persistDashboardToRoom(
            userId: String,
            plan: com.glowup.ai.domain.model.Plan,
            vertical: String,
            dto: DashboardDto,
        ...
        private suspend fun readDashboardFromRoom(
            userId: String,
            plan: com.glowup.ai.domain.model.Plan,
            vertical: String,
        ): Cached<Dashboard>? {
        ...
        suspend fun getEngagement(userId: String): GlowResult<Cached<Engagement>> {
            ...
            return when (
                ...
                is GlowResult.Failure -> {
                    if (stale != null) GlowResult.Success(stale.copy(refreshError = result.error)) else result
                }
            }
        }
        ...
        ): GlowResult<Unit> = apiCall { api.logEngagementEvent(userId, request.toDto()) }.map { }

        suspend fun getCaptureGuide(
            userId: String,
            vertical: String = "skin",
        ): GlowResult<CaptureGuide> {
        ...
        suspend fun getHistory(
            userId: String,
            vertical: String = "skin",
        ): GlowResult<Cached<List<HistoryItem>>> {
            val plan = sessionStore.plan()
            val key = "$userId:$vertical"
                                    ...
                                    HistoryCacheEntity(
                                        ...
                                        json =
                                            NetworkJson.encodeToString(
                                                com.glowup.ai.data.remote.dto.HistoryItemDto
                                                    .serializer(),
                                                dto,
                                            ),
                                        fetchedAtMillis = System.currentTimeMillis(),
        ...
        /** Executes a server-side qualitative comparison; never synthesizes
         * numeric deltas locally when the AI result is unavailable. */
        suspend fun compareCaptures(
            userId: String,
            earlierCaptureId: String,
            laterCaptureId: String,
            vertical: String = "skin",
        ): GlowResult<ComparisonResponseDto> =
            apiCall {
                api.compareCaptures(
                    userId,
                    ComparisonCreateRequestDto(earlierCaptureId, laterCaptureId, vertical),
                )
            }
        ...
        suspend fun getComparison(
            userId: String,
            comparisonId: String,
        ): GlowResult<ComparisonResponseDto> = apiCall { api.getComparison(userId, comparisonId) }

        suspend fun getCheckIns(
            userId: String,
            limit: Int = 30,
        ): GlowResult<List<CheckIn>> = apiCall { api.getCheckIns(userId, limit).map { it.toDomain() } }
        ...
        suspend fun getWeeklyRecap(
            ...
            vertical: String = "skin",
            asOf: String? = null,
        ): GlowResult<WeeklyRecap> = apiCall { api.getWeeklyRecap(userId, vertical, asOf).toDomain() }
        ...
        private suspend fun scheduleReminderFrom(
            nextAt: String?,
            cadenceDays: Int?,
        ) {
            ...
            val delayMillis =
                nextAt?.let { parseIsoToEpochMillis(it) }?.let { it - System.currentTimeMillis() }
        ...
        private suspend fun persistEngagementToRoom(
            userId: String,
            plan: com.glowup.ai.domain.model.Plan,
            dto: com.glowup.ai.data.remote.dto.EngagementDto,
        ) {
            engagementCacheDao.upsert(
                EngagementCacheEntity(
                    ...
                    json =
                        NetworkJson.encodeToString(
                            com.glowup.ai.data.remote.dto.EngagementDto
                                .serializer(),
                            dto,
                        ),
                    fetchedAtMillis = System.currentTimeMillis(),
        ...
        private suspend fun readEngagementFromRoom(
            userId: String,
            plan: com.glowup.ai.domain.model.Plan,
        ): Cached<Engagement>? {
        ...
        private suspend fun readHistoryFromRoom(
            userId: String,
            vertical: String,
        ): Cached<List<HistoryItem>>? {
