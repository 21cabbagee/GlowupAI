...
class CaptureViewModelTest {
    ...
    @Test
    fun `submitCapture with valid quality succeeds`() = runTest {
        // Given
        val mockCapture = Capture(
            id = "capture_123",
            userId = "user_123",
        ...
        val goodQuality = CaptureQuality(
            facePresent = true,
            yawDegrees = 0f,
            pitchDegrees = 0f,
            distanceCm = 45f,
    ...
    @Test
    fun `submitCapture with bad quality fails validation`() = runTest {
        // Given - face not present
        val badQuality = CaptureQuality(
            facePresent = false,
            yawDegrees = 0f,
            pitchDegrees = 0f,
            distanceCm = 45f,
    ...
    @Test
    fun `submitCapture shows uploading state`() = runTest {
        // Given
        coEvery {
            captureRepository.submitCapture(any(), any(), any())
        } coAnswers {
    ...
    @Test
    fun `submitCapture handles network error`() = runTest {
        // Given
        coEvery {
            captureRepository.submitCapture(any(), any(), any())
    ...
    @Test
    fun `quality validation checks all parameters`() = runTest {
        // Test each quality parameter
        val invalidQualities = listOf(
    ...
    @Test
    fun `baseline capture is marked correctly`() = runTest {
        // Given
        val mockCapture = Capture(
            id = "baseline_123",
            userId = "user_123",
    ...
    @Test
    fun `multiple rapid submissions are handled safely`() = runTest {
        // Given
        coEvery {
            captureRepository.submitCapture(any(), any(), any())
