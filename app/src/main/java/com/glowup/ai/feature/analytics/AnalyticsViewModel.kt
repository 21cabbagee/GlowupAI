...
class AnalyticsViewModel
    ...
    constructor(
                    ...
                    historyResult is GlowResult.Failure -> {
                        ...
                        val overview =
                            calculateOverviewStats(
                                history,
                                dashboard?.engagement?.captureStreak ?: 0,
                                experiments.count {
                                    it.status.name ==
                                        "RUNNING"
                                },
        ...
        private fun calculateTrendData(history: List<HistoryItem>): TrendData {
            ...
            val changePercent =
                if (beforeValue != null && afterValue != null && beforeValue != 0.0) {
                ...
                if (changePercent < -10) {
                    insights.add(
                        AiInsight(
                            ...
                            description = "Your redness has improved significantly over time. Recent measurements show a ${String.format(
                                "%.1f",
                                abs(changePercent),
                            )}% reduction compared to your baseline.",
                ...
                if (captureRate >= 80) {
                    insights.add(
                        AiInsight(
                            ...
                            description = "You've captured ${captureDates.size} times over $totalDays days, maintaining ${String.format(
                                "%.0f",
                                captureRate,
                            )}% consistency.",
        ...
        private fun calculateProductEffectiveness(
            ...
            experiments.filter { it.captures.size >= 3 }.map { experiment ->
