(() => {
  ...
  function onboardingFrame(content, options = {}) {
    ...
    app.innerHTML = `<main class="onboarding-shell"><div class="onboarding-inner">
      <div class="onboarding-brand">
        <span class="wordmark">Glow<em>Up AI</em></span>
  ...
  function renderOnboarding() {
    ...
    switch (d.step) {
      ...
      case 12:
        ...
        break;
      case 13:
        onboardingFrame(`<div class="profile-mark">${esc(initials(name))}</div><span class="eyebrow">YOUR SPACE IS READY</span><h1>Welcome home, ${esc(name)}.</h1><p class="lead">Your profile now has a focus, a reason, and a starting line. From here, GlowUp AI gives you one useful next step at a time.</p><div class="onboarding-actions"><span></span><button class="primary" data-action="enter-app">See my space ${icon("arrow")}</button></div>`, { cardClass: "has-signal" });
        break;
      default:
        setStep(0);
    }
  ...
  function shell(content) {
    ...
    app.innerHTML = `<div class="app-shell">
      ...
      <div>
        <header class="topbar"><span class="wordmark">Glow<em>Up AI</em></span><button class="icon-button" data-action="navigate" data-route="you" aria-label="Open profile">${esc(initials(name))}</button></header>
  ...
  function renderToday() {
    ...
    return `<section class="page-header"><div><span class="eyebrow">${esc(title(focus()))} · YOUR PRIVATE SPACE</span><h1>${greeting()}, ${esc(name)}.</h1><p class="lead">${guide.message || "Your next useful step is ready when you are."}</p></div><button class="icon-button" data-action="open-sheet" data-sheet="coach" aria-label="Ask GlowUp AI">${icon("coach")}</button></section>
  ...
  function renderSheet() {
    ...
    if (state.sheet === "consent") {
      sheet.innerHTML = `${sheetHead("A CLEAR CHOICE", "Your face is yours")}<p>GlowUp AI stores only captures you choose and creates cosmetic measurements from them. You can export or delete your history.</p><label class="check-row"><input id="workspaceConsentCheck" type="checkbox"><span>I understand and consent to facial-data capture for my GlowUp AI profile.</span></label><div class="actions"><button class="primary" data-action="grant-workspace-consent" disabled>I consent</button></div>`;
    }
  }
