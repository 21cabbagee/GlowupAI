...
sealed interface CaptureGateState {
    ...
    data class ConsentLocked(val nextAction: NextAction) : CaptureGateState
    data class Ready(val guide: CaptureGuide?) : CaptureGateState
    data class GateError(val error: ApiError) : CaptureGateState
}
...
sealed interface CapturePhase {
    ...
    data class Accepted(val captureId: String) : CapturePhase
    data class Rejected(val coaching: List<CoachingTip>, val failedChecks: List<String>) : CapturePhase
