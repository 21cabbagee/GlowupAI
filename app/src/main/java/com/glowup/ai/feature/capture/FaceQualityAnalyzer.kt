...
class FaceQualityAnalyzer(
    private val onResult: (LiveFaceQuality) -> Unit,
    ...
    override fun analyze(imageProxy: ImageProxy) {
        ...
        val input =
            runCatching { InputImage.fromMediaImage(mediaImage, rotation) }.getOrElse {
                imageProxy.close()
                return
            }
        detector
...
data class LiveFaceQuality(
    val pose: CapturePose,
    val faceFillFraction: Float,
) {
    ...
    companion object {
        fun noFace() = LiveFaceQuality(CapturePose(false, 0.0, 0.0, 0.0, false), 0f)
    }
}
