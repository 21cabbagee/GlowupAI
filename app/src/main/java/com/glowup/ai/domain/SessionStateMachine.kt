...
object SessionStateMachine {
    ...
    fun onSignInRequested(current: SessionState): SessionState = when (current) {
    ...
    fun onAuthenticationFailed(current: SessionState): SessionState = when (current) {
    ...
    fun onAuthenticationSucceeded(current: SessionState): SessionState = when (current) {
    ...
    fun onProfileRefreshRequested(current: SessionState): SessionState = when (current) {
     ...
     */
    fun onProfileResult(result: GlowResult<Profile>): SessionState = when (result) {
        is GlowResult.Success -> classify(result.data)
    ...
    private fun classify(profile: Profile): SessionState = when (profile.user.consentState) {
        ConsentState.DECLINED -> SessionState.ConsentDeclined(profile)
        ConsentState.ACTIVE ->
        ...
        // it disabled).
        ConsentState.PENDING, ConsentState.UNKNOWN -> SessionState.ConsentRequired(profile)
    }
     ...
     */
    private fun hasBaseline(profile: Profile): Boolean =
        profile.appearanceProfiles.any { it.baselineCaptureId != null }
}
