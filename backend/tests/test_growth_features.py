...
class GrowthFeatureTests(unittest.TestCase):
    def setUp(self):
        ...
        settings = Settings(
            db_path=Path(self.temp.name) / "growth.sqlite3",
            photo_dir=None,
            gemini_api_key=None,
    ...
    def create_product(
        self, name: str, stabilization_days: int = 14, ingredients=None
    ) -> dict:
    ...
    def seed_verdict(
        ...
        product_id: str,
        start: datetime,
        direction: str = "investigate",
        before_offsets=(5, 3, 1),
        after_offsets=(1, 3, 5),
    ) -> None:
        ...
        for offset in before_offsets:
            self.service.create_capture(
                user_id,
                image_bytes(),
                captured_at=(start - timedelta(days=offset)).isoformat(),
            )
        ...
        for offset in after_offsets:
            ...
            self.service.create_capture(
                user_id, img, captured_at=(start + timedelta(days=offset)).isoformat()
            )
    ...
    def test_confound_warning_flags_overlapping_stabilization_windows(self):
        ...
        first = self.client.post(
            "/api/routine-events",
            json={"user_id": user_id, "product_id": product_a["id"], "action": "start"},
        )
        ...
        self.assertEqual(
            direct_check.json()["active_windows"][0]["product_name"], "Confound Serum A"
        )

        # Starting a second product while A's window is open should warn.
        second = self.client.post(
            "/api/routine-events",
            json={"user_id": user_id, "product_id": product_b["id"], "action": "start"},
        )
    ...
    def _seed_free_tier_verdict_scenario(self):
        ...
        product_c = self.create_product(
            "Unlock Serum C (unclear)", stabilization_days=0
        )
        self.seed_verdict(
            user_id,
            product_a["id"],
        ...
        self.service.add_routine_event(
            ...
            product_c["id"],
            "start",
            datetime(2026, 3, 1, tzinfo=timezone.utc).isoformat(),
        )
        return user_id, product_a["id"], product_b["id"], product_c["id"]

    def test_free_tier_unlocks_exactly_one_definitive_verdict(self):
        user_id, product_a_id, product_b_id, product_c_id = (
            self._seed_free_tier_verdict_scenario()
        )
        ...
        definitive_labels = [
            by_product[product_a_id]["label"],
            by_product[product_b_id]["label"],
        ]
        self.assertEqual(sorted(definitive_labels), sorted(["investigate", "locked"]))
        locked_item = (
            by_product[product_a_id]
            if by_product[product_a_id]["label"] == "locked"
            else by_product[product_b_id]
        )
        ...
        self.assertNotEqual(
            by_product[product_c_id]["generated_text"],
    ...
    def test_free_tier_quota_stays_unlocked_on_the_same_product(self):
        ...
        unlocked_first = next(
            ...
            if item["product_id"] in (product_a_id, product_b_id)
            and item["label"] != "locked"
        )

        second = self.client.get(f"/api/users/{user_id}/dashboard").json()
        unlocked_second = next(
            item["product_id"]
        ...
        locked_count = sum(
            1 for item in second["verdicts"] if item["label"] == "locked"
        )
    ...
    def test_free_tier_history_is_capped_to_90_days_but_premium_sees_all(self):
        ...
        old_capture = self.service.create_capture(
            user_id, image_bytes(), captured_at=(now - timedelta(days=200)).isoformat()
        )
        recent_capture = self.service.create_capture(
            user_id, image_bytes(), captured_at=(now - timedelta(days=1)).isoformat()
        )
    ...
    def test_predict_product_is_premium_gated_and_reports_ingredient_overlap(self):
        ...
        owned_product = self.create_product(
            "Own Investigate Serum",
            stabilization_days=0,
            ingredients="Niacinamide, Retinol",
        )
        candidate_product = self.create_product(
            "Candidate Toner",
            stabilization_days=0,
        ...
        blocked = self.client.get(
            f"/api/products/{candidate_product['id']}/predict?user_id={free_user_id}"
        )
        self.assertEqual(blocked.status_code, 403)

        premium_user_id = self.create_user(premium=True)
        owned_for_premium = self.create_product(
            "Premium Investigate Serum",
            stabilization_days=0,
            ingredients="Niacinamide, Retinol",
        )
        self.seed_verdict(
            premium_user_id,
            owned_for_premium["id"],
            datetime(2026, 1, 1, tzinfo=timezone.utc),
            direction="investigate",
        )
        ...
        prediction = self.client.get(
            f"/api/products/{candidate_product['id']}/predict?user_id={premium_user_id}"
        )
    ...
    def test_root_cause_search_is_premium_gated_and_returns_valid_shape(self):
        free_user_id = self.create_user()
        self.assertEqual(
            self.client.get(f"/api/users/{free_user_id}/root-cause").status_code, 403
        )
        ...
        for index in range(6):
            self.service.create_capture(
                user_id,
                image_bytes(),
                captured_at=(now - timedelta(days=30 - index * 5)).isoformat(),
            )
            self.client.post(
                f"/api/users/{user_id}/context-events",
                json={
                    "event_type": "sleep",
                    "occurred_at": (now - timedelta(days=29 - index * 5)).isoformat(),
                },
            )
    ...
    def test_budget_optimizer_is_premium_gated_and_estimates_cost_for_stable_keep(self):
        free_user_id = self.create_user()
        self.assertEqual(
            self.client.get(f"/api/users/{free_user_id}/budget-optimizer").status_code,
            403,
        )
    ...
    def test_derm_export_is_premium_gated_and_returns_printable_html(self):
        free_user_id = self.create_user()
        self.assertEqual(
            self.client.get(f"/api/users/{free_user_id}/derm-export").status_code, 403
        )
    ...
    def test_experiment_early_stop_flags_conclusive_evidence_before_target_days(self):
        ...
        fresh_product = self.create_product(
            "No Evidence Yet Serum", stabilization_days=0
        )
        fresh_experiment = self.client.post(
            "/api/experiments",
            json={
                "user_id": user_id,
        ...
        fresh_detail = self.client.get(
            f"/api/users/{user_id}/experiments/{fresh_experiment['id']}"
        ).json()
