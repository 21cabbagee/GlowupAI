...
class HomeViewModelTest {
    ...
    @Test
    fun `loadDashboard updates state with dashboard data`() = runTest {
        // Given
        val mockDashboard = Dashboard(
            userId = "test_user",
            currentStreak = 5,
            longestStreak = 10,
            totalCaptures = 15,
    ...
    @Test
    fun `loadDashboard shows loading state`() = runTest {
        // Given
    ...
    @Test
    fun `loadDashboard handles error gracefully`() = runTest {
        // Given
        val errorMessage = "Network error"
        coEvery { homeRepository.getDashboard(any()) } returns flowOf(
    ...
    @Test
    fun `streak calculation is correct`() = runTest {
    ...
    @Test
    fun `empty dashboard shows zero streaks`() = runTest {
        // Given - new user with no captures
        val emptyDashboard = Dashboard(
            userId = "new_user",
            currentStreak = 0,
            longestStreak = 0,
            totalCaptures = 0,
    ...
    @Test
    fun `multiple rapid refreshes dont cause issues`() = runTest {
        // Given
    ...
    @Test
    fun `baseline comparison is available when baseline exists`() = runTest {
        // Given - dashboard with baseline
        val dashboardWithBaseline = Dashboard(
            userId = "user",
            currentStreak = 3,
            longestStreak = 5,
            totalCaptures = 10,
