...
def enrich_metric(metric: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    if metric is None:
        return None
    # Add smoothness_score as inverse of texture_score
    if "texture_score" in metric and "smoothness_score" not in metric:
        metric["smoothness_score"] = round(100.0 - metric["texture_score"], 3)
    return metric


class GlowupAIService:
    def __init__(
        self,
        db: Database,
    ...
    def require_consent(self, user_id: str) -> Dict[str, Any]:
        ...
        if user["consent_state"] != "active":
            raise PermissionError(
                "explicit facial-data consent is required before using photo capture",
            )
        return user
        ...
        try:
            self.db.execute(
                ...
                (
                    ...
                    json_dumps(parse_ingredients(ingredients)),
                    stabilization_days,
                ),
            )
    ...
    def create_capture(
        ...
        if not quality.accepted:
            raise ValueError(
                json.dumps(
                    {
                        "message": "capture quality is below the acceptance threshold",
        ...
        job_id = new_id()
        self.db.execute(
            "INSERT INTO analysis_jobs (id, capture_id) VALUES (?, ?)",
            (job_id, capture_id),
        )
        ...
        try:
            ...
            if baseline_row:
                baseline = MetricResult(
                    blemish_count=baseline_row["blemish_count"],
                    redness_score=baseline_row["redness_score"],
                    redness_delta=baseline_row["redness_delta"],
                    darkspot_area=baseline_row["darkspot_area"],
                    texture_score=baseline_row["texture_score"],
                    confidence=baseline_row["confidence"],
                    noise_floors=json.loads(baseline_row["noise_floor_json"]),
                    model_version=baseline_row["model_version"],
                )
            result = analyze(
                image_bytes,
                float(quality.get("score", 0.0)),
                None if capture["is_baseline"] else baseline,
            ...
            self.db.execute(
                ...
                (
                    ...
                    result.confidence,
                    json_dumps(result.noise_floors),
                ),
            )
            self.db.execute(
    ...
    def export_user(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "verdicts": [
                ...
                for row in self.db.fetchall(
                    "SELECT v.*, p.name AS product_name FROM verdicts v JOIN products p ON p.id = v.product_id WHERE v.user_id = ? ORDER BY v.generated_at",
                    (user_id,),
                )
            ],
