...
class CaptureQuality:
    ...
    def evaluate(self, skip_checks: bool = False) -> "CaptureQuality":
        ...
        if not self.face_present:
            ...
            coaching.append(
                {
                    "check": "face_not_detected",
                    "message": "Center your face in the guide frame before capturing.",
                }
            )
        if abs(self.yaw_degrees) > 12:
            ...
            coaching.append(
                {
                    "check": "turn_head_less",
                    "message": f"Turn {abs(round(self.yaw_degrees))}° back toward center ({direction}) — yaw must stay within 12°.",
                }
            )
        if abs(self.pitch_degrees) > 12:
            ...
            coaching.append(
                {
                    "check": "level_head",
                    "message": f"Tilt your chin {direction} about {abs(round(self.pitch_degrees)) - 12}° to level your head — pitch must stay within 12°.",
                }
            )
        if not 0.20 <= self.brightness <= 0.85:
            ...
            if self.brightness < 0.20:
                coaching.append(
                    {
                        "check": "adjust_lighting",
                        "message": "It's too dark for a comparable capture — move toward a window or add a front light.",
                    }
                )
            else:
                coaching.append(
                    {
                        "check": "adjust_lighting",
                        "message": "It's overexposed — step back from direct light or turn away from a bright window.",
                    }
                )
        if self.sharpness < 0.35:
            ...
            coaching.append(
                {
                    "check": "hold_phone_steady",
                    "message": "The frame is blurry — brace your elbows against your body and hold still for a second before the shutter.",
                }
            )
        if not 30 <= self.distance_cm <= 80:
            ...
            if self.distance_cm < 30:
                coaching.append(
                    {
                        "check": "move_to_capture_distance",
                        "message": f"You're too close ({round(self.distance_cm)}cm) — move back to 30-80cm from the camera.",
                    }
                )
            else:
                coaching.append(
                    {
                        "check": "move_to_capture_distance",
                        "message": f"You're too far ({round(self.distance_cm)}cm) — move closer to 30-80cm from the camera.",
                    }
                )
        if not self.expression_neutral:
            ...
            coaching.append(
                {
                    "check": "relax_expression",
                    "message": "Relax your expression to neutral — a consistent expression keeps captures comparable.",
                }
            )
